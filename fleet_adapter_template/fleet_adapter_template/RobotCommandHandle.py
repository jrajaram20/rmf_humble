...
class RobotCommandHandle(adpt.RobotCommandHandle):
    ...
    def follow_new_path(
        ...
        for x in range(len(self.remaining_waypoints)):
            self.node.get_logger().info(str(x))
        ...
        def _follow_path():
            ...
            while (
                ...
                if self.state == RobotState.IDLE:
                    ...
                    self.node.get_logger().info("into idle")
                    ...
                    with self._lock:
                        if self.target_waypoint is not None:
                            ...
                            self.node.get_logger().info("tgt way is not none")
