...
import sys
#sys.path.append('/home/rajaram/ld_rmf/src/om_aiv_msg')
#import file_1
...
class RobotState:
    def __init__(self, status: str, battery: float, location: List[float]):
        self.status = status
...
class FleetManager(Node):
    def __init__(self, config, nav_graph):
        ...
        self.create_subscription(Status, 'state', self.robot1_status_cb, 10)
        #self.create_subscription(Status, 'amr2_status', self.robot2_status_cb, 10)
        #self.create_subscription(String, 'amr1_timetogoal', self.robot_ttg, 10)
    ...
    def robot1_status_cb(self, msg):
        location = msg.location
        x = location.x
        y = location.y
        theta = location.theta
        loc = [x, y, theta]
        robot_state = RobotState(msg.status, msg.state_of_charge,loc)
        self.robots["amr1"] = robot_state
    
    def robot2_status_cb(self, msg):
        location = msg.location
        x = location.x
        y = location.y
        theta = location.theta
